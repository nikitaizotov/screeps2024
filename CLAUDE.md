...
**It is dormant.** `main.ts` calls `runKernel()` only when `Memory.kernel.enabled === true` (lazy `require`, so when off it never even loads). Default off → the old roles run live, unchanged. Modes: `off` / `observe` (build model + plan + report, command nothing) / `active` (runs executors). Migration is concern-by-concern (logistics first), each landed dormant, enabled behind a flag, verified in-game, then made default. **Do not wire kernel logic into `RoomService`/`enabledRoles`; keep the two systems separate until a concern is fully migrated.**

**Per-room takeover (Phase 2):** `Memory.kernel.takeover[roomName]` holds per-concern flags (`logistics`/`mining`/`upgrade`/`build`). The planner requests a kernel hauler crew for a room only when `takeover[room].logistics` is set, and the old `WorkerService` stands down from the `Transferring` task for that room — but **only while a kernel hauler is alive** (`kernelOwnsLogistics`), a self-healing fallback that prevents a logistics death-spiral. Enable in the in-game console: `Memory.kernel = { enabled: true, mode: "active", takeover: { "W1N1": { logistics: true } } }`; remove the flag to roll back instantly. The old roles only *read* `Memory.kernel.takeover` to stand down — that is the sole, one-directional coupling. The hauler executor caches its target id on the creep and re-selects with `findClosestByRange` (no per-tick pathfinding).
