...
const build_roads_service_1 = require("./build-roads.service");
...
const terrainCache = {};
function getRoomTerrain(roomName) {
...
class BuildService {
    constructor() {
        ...
        this.firstStructurePos = null;
        this.linkService = new link_service_1.default();
        this.containerService = new container_service_1.ContainerService();
        this.buildRoadsService = new build_roads_service_1.BuildRoadsService();
            ...
            for (let roomName in rooms) {
                ...
                if (room.controller &&
                    ...
                    if (Game.time % 15000 === 0)
                        this.buildRoadsService.planRoads(room);
                    ...
                    if (Game.time % 244 === 0)
                        this.buildRoadsService.buildRoadsAroundStructures(room);
                    // Build containers every 233 ticks.
                    if (Game.time % 233 === 0)
                        this.buildContainers(room);
                    // Build links every 244 ticks.
                    if (Game.time % 244 === 0) {
                        if (this.linkService.isLinksAvailable(room)) {
                            this.linkService.buildLinks(room);
                        }
                        this.linkService.cacheLinks(room);
                    }
                }
            }
        }
    ...
    buildContainers(room) {
        this.containerService.buildContainers(room);
    }
                    ...
                    for (let xOffset = -radius; xOffset <= radius; xOffset++) {
                        for (let yOffset = -radius; yOffset <= radius; yOffset++) {
                            ...
                            if (hasExtensionConstructionSite || hasExtension) {
                                this.buildRoadsService.buildRoadsFromFirstStructure(room, new RoomPosition(x, y, room.name));
                                found = true;
                                break;
                            }
                        }
     ...
     */
    checkConstructionSites(room, max) {
        try {
