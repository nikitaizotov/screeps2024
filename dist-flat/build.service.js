...
const container_service_1 = __importDefault(require("./container.service"));
const link_service_1 = __importDefault(require("./link.service"));
const linkService = new link_service_1.default();
const buildService = {
    structureCache: {},
    cachedPaths: [],
    exitZones: [],
    ...
    planRoads(room) {
        try {
            ...
            if (room.controller && room.controller.my) {
                ...
                const hashPos = (pos) => `${pos.roomName}_${pos.x}_${pos.y}`;
                const addConnection = (pos1, pos2) => {
                    ...
                    for (let posData of Memory.cachedPaths) {
                        ...
                        if (room && room.controller && room.controller.my) {
                            ...
                            if (!hasRoad) {
                                ...
                                let hasConstructionSite = constructionSites.some((s) => s.structureType === STRUCTURE_ROAD);
                ...
                const planRoadBetween = (pos1, pos2) => {
                    ...
                    Memory.cachedPaths.push(...path.map((pos) => ({
                        x: pos.x,
                        y: pos.y,
    ...
    processBuildOrder(room) {
        try {
            if (room.controller && room.controller.my) {
                ...
                const { spawns, constructionSites, existingStructures } = Memory.structureCache[room.name];
                if (spawns.length === 0) {
                    ...
                    for (let i = 0; i < this.buildOrder.length; i++) {
                        ...
                        const availableCount = this.getAvailableStructureCount(room, structureType, existingStructures, constructionSites);
                        if (availableCount > 0) {
    ...
    buildStructure(room, type, maxCount, constructionSites, existingStructures) {
        try {
            ...
            for (let radius = 1; structuresPlanned < maxCount; radius++) {
                for (let xOffset = -radius; xOffset <= radius; xOffset++) {
                    for (let yOffset = -radius; yOffset <= radius; yOffset++) {
                        ...
                        const areaToCheck = room.lookAtArea(y - 1, x - 1, y + 1, x + 1, true);
                        const hasContainerNearby = areaToCheck.some((a) => a.structure && a.structure.structureType === STRUCTURE_CONTAINER);
                        const hasSourceNearby = areaToCheck.some((a) => a.type === LOOK_SOURCES);
                    ...
                    for (let radius = 1; radius <= maxRadius; radius++) {
                        ...
                        for (let xOffset = -radius; xOffset <= radius; xOffset++) {
                            for (let yOffset = -radius; yOffset <= radius; yOffset++) {
                                ...
                                if (this.isRestrictedZone(exitZones, cachedPaths, x, y))
                                    continue;
                                let constructionSites = room.lookForAt(LOOK_CONSTRUCTION_SITES, x, y);
                                let structures = room.lookForAt(LOOK_STRUCTURES, x, y);
                                let hasExtensionConstructionSite = constructionSites.some((site) => site.structureType === STRUCTURE_EXTENSION);
                                let hasExtension = structures.some((structure) => structure.structureType === STRUCTURE_EXTENSION);
    ...
    buildRoadsAroundStructures(room) {
        try {
            if (room.controller && room.controller.my) {
                try {
                    ...
                    structures.forEach((structure) => {
                        ...
                        positions.forEach((pos) => {
                            ...
                            if (x >= 0 && x <= 49 && y >= 0 && y <= 49) {
                                ...
                                let isRoadPresent = look.some((lookObject) => lookObject.type === LOOK_STRUCTURES &&
                                    lookObject.structure &&
                                    lookObject.structure.structureType === STRUCTURE_ROAD);
                                let isConstructionSitePresent = look.some((lookObject) => lookObject.type === LOOK_CONSTRUCTION_SITES &&
                                    lookObject.constructionSite &&
                                    lookObject.constructionSite.structureType === STRUCTURE_ROAD);
                                let isObstacle = look.some((lookObject) => lookObject.type === LOOK_TERRAIN &&
                    ...
                    console.log(`Error buildRoadsAroundStructures in ${room.name}: ${error.message}`);
                }
            }
        }
        catch (error) {
    ...
    buildRoadsFromFirstStructure(room, startPos) {
        try {
            let sources = room.find(FIND_SOURCES);
            let controller = room.controller;
            ...
            for (let target of targets) {
                let path = PathFinder.search(startPos, { pos: target, range: 1 }, {
                    plainCost: 2,
                    swampCost: 10,
                    roomCallback: function (roomName) {
                        let room = Game.rooms[roomName];
    ...
    isRestrictedZone(exitZones, cachedPaths, x, y) {
        try {
    ...
    isValidConstructionPosition(room, x, y, constructionSites, existingStructures) {
        try {
    ...
    getAvailableStructureCount(room, structureType, existingStructures, constructionSites) {
        var _a, _b;
        try {
            ...
            const existingCount = existingStructures.filter((s) => s.structureType === structureType).length;
            const constructionCount = constructionSites.filter((s) => s.structureType === structureType).length;
        ...
        try {
            ...
            if (!Memory.roomData.exits[room.name]) {
                Memory.roomData.exits[room.name] = {
                    ...
                    [FIND_EXIT_RIGHT]: this.getExitRampPoint(room, FIND_EXIT_RIGHT, spawn),
                    [FIND_EXIT_BOTTOM]: this.getExitRampPoint(room, FIND_EXIT_BOTTOM, spawn),
                    ...
                    for (let cluster of clusters) {
                        for (let exitPosition of cluster) {
                            ...
                            for (const pos of wallPositions) {
                                const rampartPositions = Memory.roomData.exits[room.name][exitType];
                                const structuresAtPos = room.lookForAt(LOOK_STRUCTURES, pos.x, pos.y).length === 0;
                                const constructionSitesAtPos = room.lookForAt(LOOK_CONSTRUCTION_SITES, pos.x, pos.y)
                                    .length === 0;
                                if (structuresAtPos && constructionSitesAtPos) {
                                    const isRampartPosition = rampartPositions.some((p) => p.x === pos.x && p.y === pos.y);
    ...
    getExitRampPoint(room, exitType, spawn) {
        const returnData = [];
        const exitPositions = room.find(exitType);
        ...
        exitPositions.sort((a, b) => exitType === FIND_EXIT_TOP || exitType === FIND_EXIT_BOTTOM
            ? a.x - b.x
            : a.y - b.y);
        ...
        for (let centralPoint of centralPoints) {
            const position = new RoomPosition(centralPoint.x, centralPoint.y, room.name);
    ...
    addRampartPosition(primaryPosition, secondaryPosition, returnData, path) {
        const primaryRampartPosition = this.getPointOnPath(primaryPosition[0], primaryPosition[1], path);
        if (primaryRampartPosition) {
            returnData.push(primaryRampartPosition);
        }
        else {
            const secondaryRampartPosition = this.getPointOnPath(secondaryPosition[0], secondaryPosition[1], path);
